...
// Rules earlier template versions shipped and V6 deliberately dropped. They are
// removed on merge so an upgrade actually lifts them. (V5 blocked every push;
// V6 blocks only force pushes.)
const RETIRED_RULES = { deny: ['Bash(git push *)', 'Bash(git push)', 'Bash(git push:*)'] };
...
function mergeSettings(existing, template) {
  ...
  for (const key of ['allow', 'deny', 'ask']) {
    ...
    const retiring = RETIRED_RULES[key] || [];
    retired.push(...(perms[key] || []).filter(rule => retiring.includes(rule)));
    let merged = union((perms[key] || []).filter(rule => !retiring.includes(rule)), template.permissions[key]);
...
function install({ targetDir, tokens, mode, dryRun = false, skipDeps = false }) {
  ...
  for (const rel of walk(TEMPLATES_DIR)) {
    ...
    if (kind === 'json') {
      ...
      const { settings: merged, retired } = mergeSettings(current, template);
      const note = 'permissions/hooks merged, your entries kept' +
        (retired.length ? '; removed old template rule(s): ' + retired.join(', ') : '');
