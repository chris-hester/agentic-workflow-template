...
//   json     — .claude/settings.json and .mcp.json are merged, never replaced.
...
function mergeMcp(existing, template) {
  const servers = { ...(existing.mcpServers || {}) };
  const added = [];
  for (const [name, def] of Object.entries(template.mcpServers || {})) {
    if (!servers[name]) { servers[name] = def; added.push(name); }
  }
  return { content: { ...existing, mcpServers: servers }, added };
}
...
function install({ targetDir, tokens, mode, dryRun = false, skipDeps = false }) {
  ...
  for (const rel of walk(TEMPLATES_DIR)) {
    ...
    if (kind === 'json') {
      ...
      let merged; let note;
      if (rel === '.mcp.json') {
        const r = mergeMcp(current, template);
        merged = r.content;
        note = r.added.length ? 'added servers: ' + r.added.join(', ') : '';
      } else {
        merged = mergeSettings(current, template);
        note = 'permissions/hooks merged, your entries kept';
      }
...
module.exports = {
  ...
  install,
  // exported for tests
  mergeClaudeMd, mergeSettings, mergeMcp, mergeLines,
};
